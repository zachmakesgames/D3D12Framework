...
void DebugPass::PreRender(UINT frameNumber)
{
    auto cmdList = Dx12Device::GetCommandList();

    PIXBeginEvent(cmdList.Get(), 0, "Debug");
    ...
    cmdList->SetPipelineState(mResourceGroup->mPSOs["debugPSO"_h].Get());

    PIXEndEvent(cmdList.Get());
}

void DebugPass::Render(UINT frameNumber)
{
    auto cmdList = Dx12Device::GetCommandList();
    ...
    for (RenderObject* ref : mRenderObjectRefs)
    {
        ...
        cmdList->IASetPrimitiveTopology(D3D12_PRIMITIVE_TOPOLOGY::D3D10_PRIMITIVE_TOPOLOGY_LINELIST);
...
void DebugPass::PostRender(UINT frameNumber)
{
    auto cmdList = Dx12Device::GetCommandList();
